// Upstream ref discovery for pointer skills — `git ls-remote` (and, for skills-hub origins, the
// registry's version list). SHARED because it is an SSRF-sensitive sink used by BOTH the web
// propose form (/api/pointer/refs) and the worker's §29 `list_upstream_refs` MCP tool: the URL
// validator, the DNS private-IP re-check, the https-only transport allowlist, no-redirects, no
// credential prompt and the hard timeout must exist exactly once.
import { spawn } from "node:child_process";
import { lookup } from "node:dns/promises";
import { validatePointerUrl, isBlockedIp } from "./net.js";
import { isSkillsHubUrl, parseSkillsHubApiUrl, skillsHubApiUrl } from "./skills-hub.js";
